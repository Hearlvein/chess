...
public:
	...
	void renderPieces()
	{
		for (size_t y = 0; y < 8; ++y)
		{
			for (size_t x = 0; x < 8; ++x)
			{
				auto& genericPiece = m_resMgr.getPieceSprite(m_board[x][y].type, m_board[x][y].color);
	...
	bool applyMove(int xDest, int yDest)
	{
		...
		Move currentMove(Coords(m_turnInfo.clickedSquare.x, m_turnInfo.clickedSquare.y), Coords(xDest, yDest));
